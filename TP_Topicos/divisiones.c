...
{
    filtroIPC filtro;
    solicitarDatos(&filtro, selecMonto, selecRegion, selecPeriodoDiv);
...
int buscarIndiceEnVector(Vector *v, char *fecha, char *region)
{
    ...
    while (!vectorIteradorFin(&it))
    {
        if (strcmp(ac->fecha, fecha) == 0 && strcmp(ac->region, region) == 0)
            return pos;
        ac = (acumuladorRegion *)vectorIteradorSiguiente(&it);
        pos++;
    }
...
{
    ipc_div *i = (ipc_div *)elem;
    printf("%12s;%56s;%36s;%10s;%6s;%6s;%10s;%15s\n",
        i->code, i->desc, i->clasificador, i->indice_ipc,
        i->v_m_ipc, i->v_a_ipc, i->region, i->periodo);
}

void mostrarDivisiones(const void *elem)
{
    divisiones *i = (divisiones *)elem;
    printf("%15s;%55s;%12s;%12s;%12s\n",
           i->fecha, i->desc, i->indice_ipc, i->region, i->grupo);
}
